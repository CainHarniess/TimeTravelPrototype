namespace Osiris.TimeTravelPuzzler.Player
{
    internal struct AssetMenu
    {
        public const string MenuRoot = "ScriptableObjects/Player/";
        
        public const string MovementBuilderFileName = "New Movement Builder";
        public const string MovementBuilderPath = MenuRoot + "MovementBuilder";

        public const string MovementBuildDirectorFileName = "New Movement Build Director";
        public const string MovementBuildDirectorPath = MenuRoot + "MovementBuilderDirector";
    }
}
